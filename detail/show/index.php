...
for($h=0;$h<=$js-1;$h++){
	$sql = "select * from goods_info where id=$goods_id[$h]";
	$result = $mysqli->query($sql);
...
HTML;
	for($h=0;$h<=$attr_value_num[$j]-1;$h++){
		echo<<<HTML
						...
						<li id="three-01">
							<a href="?id=$id[$j]&sid=$sid[$h]" >{$attribute_value_name[$j][$h]}</a>
						</li>
HTML;
}
...
HTML;
				...
				<li>
					<div class="s-01">
						<div class="s-01-pic"><a href="http://121.42.211.188/badapple/detail/index.php?goods_id=$goods_id[$h]"><img width="250" height="250" src="pic/pic_5.jpg"></a></div>
